...
class BookingListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        qs = Booking.objects.select_related(
            ...
            "vehicle",
            "trip"
        ).prefetch_related("chargingsession_set").order_by("-created_at")

        if user.role == "ADMIN":
            filtered_qs = qs.all()
        elif user.role == "OPERATOR":
            filtered_qs = qs.filter(station__operator=user)
        else:
            filtered_qs = qs.filter(user=user)

        # Performance optimization: Include all active/pending/confirmed bookings + recent 200 completed bookings
        active_ids = list(filtered_qs.filter(booking_status__in=["PENDING", "CONFIRMED"]).values_list("id", flat=True))
        recent_ids = list(filtered_qs.exclude(booking_status__in=["PENDING", "CONFIRMED"]).values_list("id", flat=True)[:200])
