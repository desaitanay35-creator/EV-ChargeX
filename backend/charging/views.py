...
class ChargerListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if user.role == "OPERATOR" and station.operator and station.operator != user:
...
class ChargingSessionListCreateView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.role == "ADMIN":
            ...
            filtered_qs = qs.filter(charger__station__operator=user)
        else:
            filtered_qs = qs.filter(booking__user=user)

        # Performance optimization: Include all ACTIVE sessions + recent 200 completed sessions
        active_ids = list(filtered_qs.filter(session_status="ACTIVE").values_list("id", flat=True))
        recent_ids = list(filtered_qs.exclude(session_status="ACTIVE").values_list("id", flat=True)[:200])
        combined_ids = set(active_ids + recent_ids)
        return filtered_qs.filter(id__in=combined_ids)
...
def stop_charging(request):
    ...
    with transaction.atomic():
        ...
        Notification.objects.create(
            ...
            message=f"Charging completed. Energy: {energy_delivered:.2f} kWh, Cost: Rs. {cost:.2f}",
            notification_type="CHARGING"
        )
