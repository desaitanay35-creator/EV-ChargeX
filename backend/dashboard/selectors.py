...
def get_admin_period_summary(range_value="6m"):
    ...
    if start_date:
        ...
        payment_qs = payment_qs.filter(created_at__gte=start_datetime)
        session_qs = session_qs.filter(created_at__gte=start_datetime)
...
def get_admin_revenue_trend(range_value="6m"):
    ...
    if range_value in ["7d", "30d"]:
        ...
        date_format = "%Y-%m-%d"
        def get_key(dt):
            return timezone.localdate(dt).strftime(date_format)
    elif range_value == "3m":
        start_date = today - timedelta(days=90)
        date_format = "%Y-%m"
        def get_key(dt):
    ...
    qs = Payment.objects.filter(payment_status="SUCCESS").order_by("created_at")
    if start_date:
        start_datetime = timezone.make_aware(datetime.combine(start_date, datetime.min.time()))
        qs = qs.filter(created_at__gte=start_datetime)

    payments_data = list(qs.values("created_at", "amount"))
    ...
    for p in payments_data:
        ...
        if key not in trend_dict:
            trend_dict[key] = {"revenue": Decimal("0.00"), "count": 0}
        trend_dict[key]["revenue"] += p["amount"]
...
def get_admin_system_alerts():
    ...
    thirty_days_ago_dt = timezone.make_aware(datetime.combine(thirty_days_ago, datetime.min.time()))
    ...
    seven_days_ago_dt = timezone.make_aware(datetime.combine(seven_days_ago, datetime.min.time()))
    interrupted_cnt = ChargingSession.objects.filter(
        session_status="INTERRUPTED",
        created_at__gte=seven_days_ago_dt
    ).count()
