...
from .demand_forecast import predict_station_demand
...
class WaitTimePredictionView(APIView):
    ...
    def post(self, request):
        ...
        predicted_wait_time = predict_wait_time(station)

        return Response({
            "predicted_wait_time": predicted_wait_time,
            "message": "Predicted waiting time computed using ML queue simulation."
        })
...
class StationDemandForecastView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        if not station_id:
            ...
            if user.role == "OPERATOR":
                first_station = Station.objects.filter(operator=user).first()
                if first_station:
        ...
        if not station_id:
            return Response({"error": "No station_id provided and no station found."}, status=400)

        result = predict_station_demand(station_id, timeframe)
        if "error" in result:
...
class RetrainMLModelsView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        from .train_demand_forecast import train_demand_forecast_model
        from .train_wait_time import train_model as train_wait_time_model
        from charging.models import ChargingSession

        try:
            train_demand_forecast_model()
            train_wait_time_model()
            session_count = ChargingSession.objects.filter(session_status="COMPLETED").count()
            return Response({
                "message": f"Successfully re-trained all ML models on {session_count} completed session records.",
                "total_trained_sessions": session_count,
                "status": "Success"
            })
