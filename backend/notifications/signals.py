    ...
    if instance.session_status == "COMPLETED":
        ...
        if not exists:
            Notification.objects.create(
                user=user,
                title="Charging Session Completed",
                message=f"Charging session #{instance.id} completed. Battery charged to {instance.battery_after}%. Energy consumed: {instance.energy_consumed_kwh} kWh. Total cost: Rs. {instance.charging_cost}.",
                notification_type="CHARGING"
            )
    ...
    if created or instance.payment_status in ["SUCCESS", "FAILED"]:
        ...
        if not exists:
            Notification.objects.create(
                user=instance.user,
                title=title,
                message=f"Payment of Rs. {instance.amount} {status_text} (Transaction: {instance.transaction_id}).",
