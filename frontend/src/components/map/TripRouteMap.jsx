...
function MapBoundsController({ origin, destination, polylinePoints, candidateStations = [], reservePoint, stops = [] }) {
  const map = useMap();
  ...
  }, [map, origin, destination, polylinePoints, candidateStations, reservePoint, stops]);
  ...
  return (
    <div className="trip-route-map-wrapper" style={{ height: "420px", width: "100%", borderRadius: "12px", overflow: "hidden" }}>
      ...
      >
        ...
        {/* Render ONLY ONE Route Polyline: Multi-stop route via charging stations if stops exist, else direct route */}
        {detourPolylinePoints.length > 0 ? (
          <Polyline
            positions={detourPolylinePoints}
            pathOptions={{ color: "#2563eb", weight: 6, opacity: 0.95, lineCap: "round" }}
          />
        ...
        {stops.map((stop, idx) => {
          ...
          return (
            <Marker
              key={`stop-${idx}-${stop.station_id || idx}`}
              position={[Number(lat), Number(lng)]}
              icon={selectedStationIcon}
            >
              <Popup>
                <div style={{ minWidth: "200px" }}>
                  <strong style={{ color: "#b45309", fontSize: "0.9rem" }}>Stop #{idx + 1}: Charging Station</strong>
                  <h4 style={{ margin: "4px 0", color: "var(--text-primary)" }}>{stop.station_name}</h4>
                  <div style={{ display: "flex", flexDirection: "column", gap: "2px", fontSize: "0.8rem", marginTop: "6px" }}>
                    <span>📍 Address: {stop.address}, {stop.city}</span>
                    <span>🛣️ Leg Distance: <strong>{stop.distance_from_previous_km} km</strong></span>
                    <span>🔋 Arrival Battery: <strong>{stop.arrival_battery_percentage}%</strong></span>
                    <span>⚡ Charge: <strong>{stop.arrival_battery_percentage}% ➜ 100%</strong></span>
                    <span>⏱️ Charging Time: <strong>{stop.estimated_charging_time_minutes} mins</strong></span>
                    {stop.wait_time_minutes > 0 && <span>⏱️ Wait Time: <strong>{stop.wait_time_minutes} mins</strong></span>}
                    <span>💰 Est. Cost: <strong>₹{stop.estimated_cost}</strong></span>
                  </div>
                </div>
              </Popup>
            </Marker>
