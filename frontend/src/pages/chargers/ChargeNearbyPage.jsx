...
import { FaBolt, FaCar, FaCrosshairs, FaFilter, FaMapMarkerAlt, FaRoute, FaSearch, FaStar } from "react-icons/fa";
    ...
    return data.stations
      ...
      .filter((st) => {
        ...
        if (!showAllStations) {
          ...
          if (st.availableCompatibleCount === 0) return false;
        }
        return true;
      })
      .sort((a, b) => {
    ...
    <section className="charge-nearby-container" style={{ padding: "16px 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(340px, 1fr) minmax(360px, 1.2fr)", gap: "20px" }}>
        ...
        <div className="nearby-map-wrapper" style={{ height: "600px", sticky: "top 80px" }}>
          <TripRouteMap
            sourceLocation={userLocation ? { lat: userLocation.lat, lng: userLocation.lng, name: userLocation.name } : null}
            destinationLocation={selectedStation ? { lat: selectedStation.lat, lng: selectedStation.lng, name: selectedStation.station_name } : null}
            stations={filteredNearbyStations}
            suggestedStation={selectedStation}
