  ...
  return (
    <section>
      <PageHeader
        eyebrow="Billing"
        title="Payments"
        description="Settle completed charging sessions and keep every transaction in one place."
      />
      <div className="summary-strip">
        <div>
          <span className="summary-icon"><FaWallet /></span>
          <span>
        ...
        <div>
          <span className="summary-icon warning"><FaRupeeSign /></span>
          <span>
        ...
        <div>
          <span className="summary-icon blue"><FaReceipt /></span>
          <span>
      ...
      {payments.length ? (
          ...
          {payments.map((payment) => (
            <article className="payment-card" key={payment.id}>
              ...
              <div className="payment-main">
                <p>Payment #{payment.id} · Session #{payment.charging_session || "—"}</p>
                <h2>{formatCurrency(payment.amount)}</h2>
                <span>{payment.transaction_id || "Transaction ID pending settle"}</span>
              </div>
              <div className="payment-meta">
                <StatusBadge value={payment.payment_status} />
                <span>{formatDate(payment.paid_at || payment.created_at, { hour: "numeric", minute: "2-digit" })}</span>
                <small>{payment.payment_method ? titleCase(payment.payment_method) : "Method not selected"}</small>
              </div>
        ...
        <EmptyState
          ...
          message="A payment is generated when a charging session is completed."
        />
      )}
      ...
      {selected && (
        <Modal
          ...
          description={`Pay ${formatCurrency(selected.amount)} for charging session #${selected.charging_session}.`}
        ...
        >
          <form className="form-grid" onSubmit={pay}>
            ...
            <div className="payment-confirm field-full">
              <span>Amount due</span>
              <strong>{formatCurrency(selected.amount)}</strong>
              <small>This project uses a simulated payment flow; no real money is charged.</small>
            </div>
            <FormActions
              loading={saving}
              onCancel={() => setSelected(null)}
              submitLabel={saving ? "Processing payment..." : "Confirm payment"}
            />
          </form>
        </Modal>
      )}
