...
const evService = {
  ...
  trips: {
    ...collection("/trips/"),
    plan: (payload) => unwrap(api.post("/trips/plan/", payload)),
    get: (id) => unwrap(api.get(`/trips/${id}/`)),
    start: (id, payload) => unwrap(api.post(`/trips/${id}/start/`, payload)),
    end: (id, payload) => unwrap(api.post(`/trips/${id}/end/`, payload)),
    continueToDestination: (id) => unwrap(api.post(`/trips/${id}/continue-to-destination/`)),
  },
  ...
  admin: {
    users: {
      ...
      unblock: (id) => unblock(api.post(`/auth/admin/users/${id}/unblock/`)),
  ...
  getDemandForecast: (stationId, timeframe = "day") =>
    unwrap(
      api.get("/ml/demand-forecast/", {
        params: { station_id: stationId, timeframe },
      })
    ),
  retrainMLModels: () => unwrap(api.post("/ml/retrain/")),
};
