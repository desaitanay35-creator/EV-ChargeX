 ...
 * Provides geolocation & Nominatim geocoding utilities.
 */
...
/**
 * Searches locations using Nominatim Geocoding API.
 * @param {string} query
...
export const geocodeAddress = async (query, signal) => {
  ...
  const url = `${NOMINATIM_BASE_URL}/search?format=json&q=${encodeURIComponent(query.trim())}&limit=5`;
  try {
